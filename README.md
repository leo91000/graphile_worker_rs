...
When building from source, the admin UI assets are generated by Cargo from the
separate Leptos client crate. That build needs `npm`, `wasm-bindgen`, and the
`wasm32-unknown-unknown` Rust target available.
