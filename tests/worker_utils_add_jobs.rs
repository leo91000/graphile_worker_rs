use graphile_worker::{
    worker_utils::{CleanupTask, RescheduleJobOptions},
...
#[tokio::test]
async fn context_ext_exposes_worker_utils_methods() {
    with_test_db(|test_db| async move {
        #[derive(Clone, Serialize, Deserialize)]
        ...
        impl TaskHandler for UtilitySpawnerJob {
            ...
            async fn run(self, ctx: WorkerContext) -> impl IntoTaskHandlerResult {
                ...
                let completable = ctx
                    ...
                    .await
                    .map_err(|error| error.to_string())?;
                ctx.complete_jobs(&[*completable.id()])
                    .await
                    .map_err(|error| error.to_string())?;
                ...
                let permanent = ctx
                    ...
                    .await
                    .map_err(|error| error.to_string())?;
                ctx.permanently_fail_jobs(&[*permanent.id()], "manual failure")
                    .await
                    .map_err(|error| error.to_string())?;
                ...
                ctx.force_unlock_workers(&["ctx-ext-worker"])
                    .await
                    .map_err(|error| error.to_string())?;
                ctx.cleanup(&[CleanupTask::GcJobQueues])
                    .await
                    .map_err(|error| error.to_string())?;
                ctx.migrate().await.map_err(|error| error.to_string())?;
