alter type :ARCHIMEDES_SCHEMA.job_spec alter attribute max_attempts type smallint;
alter type :ARCHIMEDES_SCHEMA.job_spec alter attribute priority type smallint;

drop function :ARCHIMEDES_SCHEMA.add_job;
CREATE FUNCTION :ARCHIMEDES_SCHEMA.add_job(identifier text, payload json DEFAULT NULL::json, queue_name text DEFAULT NULL::text, run_at timestamp with time zone DEFAULT NULL::timestamp with time zone, max_attempts smallint DEFAULT NULL::smallint, job_key text DEFAULT NULL::text, priority smallint DEFAULT NULL::smallint, flags text[] DEFAULT NULL::text[], job_key_mode text DEFAULT 'replace'::text) RETURNS :ARCHIMEDES_SCHEMA.jobs
    LANGUAGE plpgsql
    AS $$
declare
...
CREATE FUNCTION :ARCHIMEDES_SCHEMA.reschedule_jobs(job_ids bigint[], run_at timestamp with time zone DEFAULT NULL::timestamp with time zone, priority smallint DEFAULT NULL::smallint, attempts smallint DEFAULT NULL::smallint, max_attempts smallint DEFAULT NULL::smallint) RETURNS SETOF :ARCHIMEDES_SCHEMA.jobs
    LANGUAGE sql
    AS $$
