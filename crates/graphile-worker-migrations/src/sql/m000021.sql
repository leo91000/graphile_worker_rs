-- Port graphile/worker#623 while preserving Rust migration 19 key locking.
...
-- Also support upgrades from upstream revision 20, which has no recovery objects.
CREATE TABLE IF NOT EXISTS :GRAPHILE_WORKER_SCHEMA._private_workers (
    id text PRIMARY KEY,
...
CREATE INDEX IF NOT EXISTS _private_workers_last_heartbeat_at_idx
    ON :GRAPHILE_WORKER_SCHEMA._private_workers (last_heartbeat_at);

-- graphile-worker-rs:statement

CREATE OR REPLACE FUNCTION :GRAPHILE_WORKER_SCHEMA.worker_heartbeat(
    worker_id text,
...
CREATE OR REPLACE FUNCTION :GRAPHILE_WORKER_SCHEMA.worker_deregister(worker_id text) RETURNS void
LANGUAGE sql
AS $$
...
CREATE OR REPLACE FUNCTION :GRAPHILE_WORKER_SCHEMA.list_stale_workers(
...
CREATE OR REPLACE FUNCTION :GRAPHILE_WORKER_SCHEMA.list_orphan_locked_workers(
    stale_threshold interval
) RETURNS TABLE(worker_id text)
LANGUAGE sql
AS $$
...
CREATE OR REPLACE FUNCTION :GRAPHILE_WORKER_SCHEMA.delete_stale_workers(
    worker_ids text[]
) RETURNS void
LANGUAGE sql
AS $$
